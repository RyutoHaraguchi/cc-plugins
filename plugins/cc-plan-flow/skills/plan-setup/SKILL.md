...
### 4. CLAUDE.md への追記
プロジェクトルートの `CLAUDE.md` を確認する。

- `CLAUDE.md` が存在しない場合: 以下の内容でファイルを作成する
- `CLAUDE.md` が存在するが、下記の内容が含まれていない場合: ファイル末尾に追記する
- 既に同等の内容が含まれている場合: 何もしない
...
IMPORTANT: プランファイル（`.claude/plans/` 配下）を作成・編集する際は、必ず `.claude/rules/plan-lifecycle.md` を読み込んでからルールに従うこと。
```
