...
- 2026-07-28(実装後の逸脱記録): 展開UIはスペックのノード上チェブロンバッジ + 「+N件」集約ノードではなく、詳細パネルの方向別展開ボタン + `dbltap` + `#expand-all`(全展開)で実装(Task 8 レビューで確定)。実行パスハイライトでパス数が表示上限20本を超過した場合のメッセージ表示は未実装(FOLLOW-UP)。
